        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Group {
                    ...
                    Text("I made this app to log weight training workouts' sets, weights, and reps more conveniently than I have found in other apps: The sets of an exercise are on one screen, and it uses picker wheels to set the numbers to log. The picker wheels start at the numbers of the previous time you logged the exercise.")
                    ...
                    Text("You should define your exercises and workouts, and I will explain below how simple that goes, though I hope that the app is clear enough without reading this whole text.")
                    Text("Your exercises are logged on your phone or icloud, and you can download or email everything you logged. You can read the tab separated (TSV) file easily into a spreadsheet for further analysis (more about the JSON file below).")
                ...
                Group {
                    ...
                    Text("When you reach the last exercise of your workout, the button 'log next' changes to 'log, end', which logs your exercise and takes you back to the start screen.")
                }
                ...
                Group {
                    ...
                    Text("When clicking on 'edit workouts', you will see a list of the workouts that you have made. You can click on a workout to edit the exercises that are in the workout. There you can define a new exercise ('new exercise') or add an exercise to the list of exercises of the workout by selecting from the list of all exercises ('Add exercise'). You can also click on 'reorder exercises' (in case you want to change the order in which your exercises are presented during this workout) by dragging an exercise with your finger on the icon with the three horizontal dashes on the right-hand side. You can also click on the red icon to the left of the name of an exercise in order to delete the exercise from this workout (it will remain in the list of all exercises).")
                    Text("Back in 'edit workouts' you can also click to reorder workouts, and on the red icon to the left of the name of a workout to delete that.")
                }

                Group {
                    Text("Edit exercises").font(.headline)
                    Text("From the start screen, you can also click on the button 'edit exercises'. This will show a list of all exercises that you have made either through this screen or when you were defining your workouts by adding a new exercise.")
                }

                Group {
                    Text("Log screen and file").font(.headline)
                    Text("The button 'logs' takes you to a screen that displays your log entries from most recent, down to older. A logged weight is green if it is larger than the weight in the previous log of the same set and exercise, and red if it is smaller. A logged number of repetitions is green if it is larger than the reps in the previous log, and the corresponding weight is not smaller; it is red if the reps number is smaller and the corresponding weight is not larger.")
                    Text("The log screen includes the following buttons:")
                    VStack(alignment: .leading, spacing: 8) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("    • export TSV - for exporting your log data as tab delimited file (TSV), e.g. for reading into a spreadsheet")
                            Text("    • export JSON - for exporting log data, and workout and exercise definitions as JSON file")
                            Text("    • import JSON - for importing a JSON file that should have the correct format (seems easiest to achieve by editing a file you exported earlier - but editing a JSON is not everybody's cup of tea)")
                            Text("    • undo import - for restoring the previous JSON if your import did not have the desired effect.")
                        }
                    }
                    Text("Export files are sent to a destination of your choice.")
                    Text("The JSON file is now the only way to correct errors or make changes in your log. I intend to make that easier at some point.")
                    Text("After importing a JSON file you can:")
                    VStack(alignment: .leading, spacing: 8) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("    • validate - check if the imported file has correct format and chronology")
                            Text("    • check counts - compare the numbers of logged items between the imported and app data")
                            Text("    • replace data - replace the app's data with the imported data")
                            Text("    • merge data - add data from the imported file that are not already in the app")
                        }
                    }
                }
                ...
                Group {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("2. You are quite welcome to provide any feedback in your review comments in the App Store, or go to GitHub and provide your comments there. Perhaps you even want to do a pull request for improvements of the code. Here are a few examples for feedback:")
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("    • Would you like to be able to change logged data, if so how would you prefer to do that?")
