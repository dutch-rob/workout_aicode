            ...
            ScrollView {
                ...
                Group {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Button { withAnimation { proxy.scrollTo("aerobic-exercises", anchor: .top) } } label: { Text("Aerobic exercises — cardio, with heart rate on the Watch").multilineTextAlignment(.leading).frame(maxWidth: .infinity, alignment: .leading) }
                ...
                Group {
                    Text("Aerobic exercises").font(.headline).id("aerobic-exercises")
                    Text("Not everything in a gym has weights on it. An exercise can be **aerobic** instead of strength: pick it from the heart-shaped **AE** button beside the body picture in the exercise library, where every indoor workout the Apple Workout app offers is waiting ready-made — indoor walk, run and cycle, elliptical, rower, stair stepper, and the rest. Take one, or make your own and choose its activity on the *edit exercise* screen.")
                    Text("An aerobic exercise has no sets, weights or muscle groups, because none of those say anything about twenty minutes on a bike. It has one wheel for how long, and a **start** button.")
                    Text("The wheel opens on however many minutes you chose for that exercise last time — twenty to begin with — so a regular session is one tap to start. It remembers what you *chose*, not what you did: stopping a twenty-minute ride early does not quietly turn it into a shorter ride for ever after.")
                    Text("Starting one runs a countdown. As with the rest timer, the clock keeps the time rather than the screen, so you can pocket the phone: it buzzes and shows a notification when the time is up, and **stop** ends it early.")
                    Text("**log, next** stays greyed out until a session has actually run, because a number on a wheel is a plan and this app logs what happened. Once it has, what the session came to appears under the exercise name — the time, your average heart rate, and how long you spent in each zone — and the button comes back. What gets logged is what you actually did: stopping at twelve minutes of a twenty-minute ride records twelve. Those same numbers are what the *logs* tab shows for that session, in place of the weights and repetitions a strength exercise has.")
                    Text("The rest timer stays quiet after cardio. You have just done twenty minutes of it, and a ninety-second rest prompt on top would be noise.")
                    Text("### With an Apple Watch If you have one, the Watch does the measuring, and it does it as a **real Apple workout**: the session is written to Health as the activity you chose, so it appears in Fitness and counts towards your rings like any other. Starting on the phone wakes the Watch app by itself — you do not have to open it.")
                    Text("While it runs, the Watch shows your heart rate and which of five zones you are in, as a coloured band. You can start on either device and stop on either; whichever you use, the other follows.")
                    Text("The zones are worked out from your resting heart rate and your age, following the method Apple describes for its automatic zones. Expect them to be close to what the Workout app shows rather than identical — Apple uses a maximum heart rate measured from your own history, which no app can read.")
                    Text("Without a Watch everything works except the heart rate. The countdown runs, the session is logged with its duration, and the screen says plainly that heart rate needs a Watch rather than leaving a blank space. The session is still written to Apple Health as a workout of the chosen activity — the Fitness app has been on iPhone in its own right since iOS 16, with a Move ring but no Exercise or Stand ring — so it counts for what it can.")
                    Text("If a Watch is there but nothing arrives from it for a while, the screen says that too, and points at Settings › Health on the Watch. It does not claim to know that permission was refused: HealthKit deliberately will not say whether a read was denied, so the app reports what it can actually see — that no readings are coming — and names the likeliest fix.")
                }
